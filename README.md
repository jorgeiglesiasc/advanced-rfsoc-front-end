...
This section details how to proceed in order to use the front-end with either a RFSoC4x2 or a ZCU208. It is recommended to use a ZCU208, since the RFSoC4x2 does not allow reaching an optimal data transmission speed with four chains. This tutorial assumes that the user have or know how to install Vivado, Vitis HLS, MATLAB and GNURadio and is using Linux. The tutorial is made with the ZCU208 but is analogous for the RFSoC4x2:
...
Vitis HLS: The project contains the codes for the two custom IP cores that were created. The synthesis clock period used is 3.33ns and the uncertainty is 10% or 5%. If any modification is made to a custom IP core in Vitis HLS it has to be exported to the _*/advanced-rfsoc-front-end/Vivado/ip_ folder.

GNURadio: If the user wants to see what signal is being sent to the final device, a script has been created in GNURadio that shows the signals processed by the four chains.

MATLAB: To check that the behavior of each custom IP core is as desired, two codes have been created in MATLAB which use the files generated by the testbench in Vitis HLS to show the behavior of the Digital Signal Processor (DSP).
