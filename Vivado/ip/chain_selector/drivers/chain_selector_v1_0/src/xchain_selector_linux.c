...
typedef struct {
    ...
    XChain_selector_uio_map maps[ MAX_UIO_MAPS ];
} XChain_selector_uio_info;

/***************** Variable Definitions **************************************/
static XChain_selector_uio_info uio_info;
...
static int uio_info_read_name(XChain_selector_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XChain_selector_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XChain_selector_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XChain_selector_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XChain_selector_Initialize(XChain_selector *InstancePtr, const char* InstanceName) {
	XChain_selector_uio_info *InfoPtr = &uio_info;
