...
typedef struct {
    ...
    XFreq_translator_uio_map maps[ MAX_UIO_MAPS ];
} XFreq_translator_uio_info;

/***************** Variable Definitions **************************************/
static XFreq_translator_uio_info uio_info;
...
static int uio_info_read_name(XFreq_translator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFreq_translator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFreq_translator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFreq_translator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFreq_translator_Initialize(XFreq_translator *InstancePtr, const char* InstanceName) {
	XFreq_translator_uio_info *InfoPtr = &uio_info;
